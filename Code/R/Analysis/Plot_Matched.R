...
dat <- list.files("../../../Data/Cleaned/Pooled/Matched")
b <- list.files("../../../Data/Cleaned/Pooled/Matched", full.names = TRUE)
...
for(i in c(1:length(dat))){
  sdat[[i]] <- sdat[[i]] %>% 
    mutate(year = Split[[i]][1],
           ...
           forumla = Split[[i]][8],
           job = Split[[i]][9],
           job = if_else(job == "Matched", "overall", job))
}
...
finalPlot_ser <- plotFun(Industry = "ser")
finalPlot_all <- plotFun(Industry = "all")

ggsave(filename = "service_matched.pdf",plot = finalPlot_ser,device = "pdf",width = 14,height = 18,
       units = c("cm"), dpi = 300,path = "../../../Final Paper/final_paper/images")

ggsave(filename = "allindustry_matched.pdf",plot = finalPlot_all,device = "pdf",width = 14,height = 18,
       units = c("cm"), dpi = 300,path = "../../../Final Paper/final_paper/images")
