...
local excel_file "rif_regression1.xlsx"
    ...
    foreach employment of local formal_employment {
        foreach q of local quantiles {
            * Run the RIF regression
            rifhdreg ln_wage $exp $education hh_size female married child_12 voc_train migrated_fr_job tot_chores_hrs urban overtime_40 $job_sector ///
                if year == `year' & formal_employment == `employment' [pweight=weight], rif(q(`q')) absorb(dist)
